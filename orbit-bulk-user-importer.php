...
add_action( 'wp_ajax_oui_save_mapping', static function(){
	...
	if ( $job_id <= 0 ) {
		wp_send_json_error( array( 'message' => __( 'Invalid job', 'orbit-import' ) ), 400 );
	}
	$mapping = isset( $_POST['mapping'] ) ? (array) $_POST['mapping'] : array();
	$bulk = isset( $_POST['bulk'] ) ? (array) $_POST['bulk'] : array();
	$clean_mapping = array();
	foreach ( $mapping as $k => $v ) {
		$clean_mapping[ sanitize_key( $k ) ] = sanitize_text_field( (string) $v );
	}
	update_post_meta( $job_id, '_mapping', $clean_mapping );

	$bulk_ids = array();
	if ( isset( $bulk['ids'] ) && is_array( $bulk['ids'] ) ) {
		foreach ( $bulk['ids'] as $id ) { $id = (int) $id; if ( $id > 0 ) { $bulk_ids[] = $id; } }
	}
	...
	$mode = isset( $bulk['mode'] ) ? sanitize_key( $bulk['mode'] ) : 'csv_only';
	if ( ! in_array( $mode, array( 'csv_only', 'append_bulk', 'replace_with_bulk' ), true ) ) { $mode = 'csv_only'; }
