(function($){
	$(function(){
		...
		$('#oui-upload-form').on('submit', function(e){
			...
			.done(function(res){
				if(res && res.success){
					alert('Uploaded. Job #' + res.data.job_id);
					$('#oui-job-id').val(res.data.job_id);
					// auto dry-run
					$.post(OUI.ajaxUrl, { action: 'oui_dry_run', nonce: OUI.nonce, job_id: res.data.job_id })
					.done(function(dr){ if(dr && dr.success){ $('#oui-dry-create').text(dr.data.created); $('#oui-dry-update').text(dr.data.updated); $('#oui-dry-invalid').text(dr.data.invalid); $('#oui-dry-unknown').text((dr.data.unknown_streams||[]).length); } });
		...
		$('#oui-save-mapping').on('click', function(){
			...
			$.post(OUI.ajaxUrl, payload).done(function(res){ if(res && res.success){ alert('Mapping saved'); } else { alert(res && res.data && res.data.message ? res.data.message : 'Error'); } });
		});

		$('#oui-run-dry').on('click', function(){
			var jobId = getJobId(); if(!jobId){ alert('No job. Upload CSV first.'); return; }
			$.post(OUI.ajaxUrl, { action: 'oui_dry_run', nonce: OUI.nonce, job_id: jobId })
			.done(function(res){ if(res && res.success){ $('#oui-dry-create').text(res.data.created); $('#oui-dry-update').text(res.data.updated); $('#oui-dry-invalid').text(res.data.invalid); $('#oui-dry-unknown').text((res.data.unknown_streams||[]).length); } else { alert(res && res.data && res.data.message ? res.data.message : 'Error'); } });
		});

		function runBatch(jobId, prevProcessed){
			$.post(OUI.ajaxUrl, { action: 'oui_run_batch', nonce: OUI.nonce, job_id: jobId })
			.done(function(res){
				if(!(res && res.success)) { alert(res && res.data && res.data.message ? res.data.message : 'Error'); return; }
				var d = res.data || {}; var percent = parseInt(d.percent||0,10);
				$('#oui-progress-bar').css('width', percent+'%'); $('#oui-progress-text').text(percent+'%');
				var processed = parseInt(d.processed||0,10);
				if(percent < 100 && processed > (prevProcessed||0)){
					setTimeout(function(){ runBatch(jobId, processed); }, 500);
				}
			});
		}
