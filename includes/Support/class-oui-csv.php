...
class CSV {
	public static function detect_dialect( $file ) {
		...
		if ( ! $handle ) { return array( 'delimiter' => ',', 'enclosure' => '"', 'escape' => '\\' ); }
		$line = fgets( $handle ); fclose( $handle );
		$line = self::strip_bom( (string) $line );
		foreach ( $delims as $d ) { $c = substr_count( $line, $d ); if ( $c > $max ) { $max = $c; $best = $d; } }
	...
	public static function iterate( $file, $delimiter = ',', $enclosure = '"', $escape = '\\' ) {
		...
		$first = true;
		foreach ( $csv as $row ) {
			if ( $first ) { $first = false; if ( is_array( $row ) && isset( $row[0] ) ) { $row[0] = self::strip_bom( (string) $row[0] ); } }
